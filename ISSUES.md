...
| # | Sev | Issue | Location |
|---|:---:|-------|----------|
| 1 | ✅ | **Fixed:** the vault is SQLCipher-encrypted with a random database key, sealed in `vault_meta.json` by an Argon2id key from the passcode. The plaintext path is gone (#42 was closed as not required: the app had no released clients). new vaults are SQLCipher-encrypted with a random database key, sealed in `vault_meta.json` by an Argon2id key from the passcode. Vaults created before this stay in plain text until they are migrated. Original report: **No encryption at rest.** Every saved login (including passwords and TOTP secrets) is stored as plain text in `logins.db`. | `lib/provider/db_helper.dart:54-76` |
| 2 | ✅ | **Fixed:** the passcode is not stored at all: the right one unwraps the database key and a wrong one fails to. The `password` column is dropped in schema v3. the passcode is not stored at all for new vaults: the right one unwraps the database key and a wrong one fails to. Legacy vaults still compare plain text. Original report: **Passcode stored and compared as plain text.** There's no hashing or key derivation (such as Argon2 or PBKDF2). | `db_helper.dart:67-73`, `screens/password_auth.dart:98`, `screens/change_password.dart:57` |
| 3 | ✅ | **Fixed:** the recovery answer is not stored; it wraps the same database key under its own salt, so recovery both proves the answer and unlocks the vault. The `answer` column is dropped in schema v3. the recovery answer is not stored for new vaults; it wraps the same database key under its own salt, so recovery both proves the answer and unlocks the vault. Legacy vaults still store the answer. Original report: **Recovery answer stored as plain text.** Recovery relies on a single knowledge question. | `db_helper.dart:130-142` |
...
| 31 | ✅ | **Fixed:** the duplicate workflow in the misspelled directory is deleted; `dart.yml` already builds Windows. Original report: `build-windows.yml` sits in `.github/workflow/` (missing "s"), so GitHub Actions never runs it. | `.github/workflow/build-windows.yml` |
| 32 | ✅ | **Fixed:** the upload path is now `build/windows/x64/runner/Release`. Original report: The CI Windows artifact path is `build/windows/runner/Release`, but Flutter ≥ 3.15 outputs to `build/windows/x64/runner/Release`, so the upload finds nothing. | `.github/workflows/dart.yml:67` |
